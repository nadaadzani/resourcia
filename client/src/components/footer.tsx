...
const Footer = () => {
  ...
  return (
    <footer className="text-black  pb-10 px-4 py-5 font-[200] text-2xl uppercase mx-auto md:px-20">
        ...
        <div className="flex-1">
          ...
          <div className=" mt-4">
            {isLoaded && (
              <GoogleMap
                zoom={15}
                center={{ lat: -6.8914747, lng: 107.6080842 }}
                mapContainerClassName=" w-[400px] h-[200px] border-0"
              >
                <Marker
                  position={{ lat: -6.8914747, lng: 107.6080842 }}
                  label={"Resourcia Warehouse"}
                />
              </GoogleMap>
            )}
          </div>
        ...
        <div className="flex-1 mt-10 space-y-6 items-center justify-between sm:flex md:space-y-0 md:mt-0">
          {footerNavs.map((item, idx) => (
            <ul className="space-y-4" key={idx}>
              <h4 className="text-black font-medium">{item.label}</h4>
